...
# 开源版不随仓库分发这些内容；缺失时页面给出准备指引而不是抛栈。
_REQUIRED_DATA = (
    ("src/ml/data/school_major_details.feather", "院校-专业基础表"),
    ("src/ml/data/cases.feather", "历史录取案例表"),
    ("src/ml/data/school_base.feather", "院校基础表"),
    ("src/ml/pre-trained_models/*.ubj.gz", "XGBoost 模型权重"),
    ("cache/background_target_similarity.feather", "背景-目标相似度矩阵"),
)
...
def _render_data_missing_notice(exc: Exception) -> None:
    """脱敏开源版不含案例数据与模型权重，缺失时给出可执行的准备指引。"""
    st.error(
        "预测所需的本地数据尚未就绪，预测链未能启动。",
        icon=":material/database:",
    )
    st.markdown(
        "本仓库为脱敏开源版，不包含原始案例库与训练产物。"
        "请将以下内容放到对应位置后刷新页面：\n\n"
        + "\n".join(f"- `{path}` — {desc}" for path, desc in _REQUIRED_DATA)
    )
    st.caption("具体缺失项：")
...
def prediction_page_content() -> None:
    ...
    try:
        ...
        _page_logger.info("数据缺失，跳过预测链 | %s", exc)
        _render_data_missing_notice(exc)
    ...
    if not session_manager.get(DEFAULT_FORM_KEYS.user_nickname):
        session_manager.set(user_nickname=user_info.get("user_nickname", "访客"))
