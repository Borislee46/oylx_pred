...
def compute_uni_classification_summary(df):
    ...
    if "国本院校分类" in df.columns:
        ...
        domestic = domestic.where(~domestic.isin(invalid_values) & domestic.notna(), "")
        parts.append(domestic)

    if "海本QS排名区间" in df.columns:
        overseas = df["海本QS排名区间"].astype(str)
        overseas = overseas.where(~overseas.isin(invalid_values) & overseas.notna(), "")
...
def main():
    parser = argparse.ArgumentParser(description="为案例库数据预计算汇总列以提升查询性能")
    parser.add_argument(
        "--data-dir",
        type=str,
