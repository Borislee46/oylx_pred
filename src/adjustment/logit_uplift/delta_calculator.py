...
class DeltaCalculator:
    ...
    def _compute_delta_logit_raw(
        ...
        if final_delta > 0 and remarks:
            ...
            if flat_remarks:
                logger.info(
                    f"[背提文本含金量] delta={final_delta:.3f}(未应用): {'; '.join(flat_remarks)}"
                )
    ...
    def get_remarks(self, sig: str) -> dict[str, list[str]]:
        import json

        details: dict = {}
        if sig and sig.startswith("{"):
            details = json.loads(sig)
        _, remarks = self._similarity_computer.compute_similarities(details)
        return {k: list(v) for k, v in remarks.items()}
...
def _fast_entropy(text: str) -> float:
    ...
    byte_rich = clip_probability(entropy / 5.0)

    n = len(text)
    if n >= 12:
        span = max(12.0, float(n**0.55))
        char_f = clip_probability(len(set(text)) / span)
        byte_rich *= 0.35 + 0.65 * char_f

    return clip_probability(byte_rich)
