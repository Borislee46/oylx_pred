...
from src.adjustment.logit_uplift import (
    DeltaCalculator,
    SimilarityComputer,
    TextProcessor,
)
...
_INTERNAL_REMARKS = frozenset({"content_novelty"})
...
class LogitUpliftProvider(TextBoostProvider):
    def __init__(
        ...
        self._sim_gate_sum_min = 0.10 if sim_gate_sum_min is None else float(sim_gate_sum_min)
        self._sim_gate_max_min = 0.08 if sim_gate_max_min is None else float(sim_gate_max_min)
        self._smoothing = 0.7 if smoothing is None else float(smoothing)
        self._cap_min_factor = 0.10 if cap_min_factor is None else float(cap_min_factor)
        self._cap_quality_gamma = 1.2 if cap_quality_gamma is None else float(cap_quality_gamma)
    ...
    def get_quality_tags(self, experience_details: dict[str, Any]) -> dict[str, list[str]]:
        if not has_any_experience(experience_details):
            return {}

        sig = self._text_processor.make_signature(experience_details)
        try:
            remarks = self._delta_calculator.get_remarks(sig)
        except (
            FileNotFoundError,
            OSError,
            EOFError,
            ...
            logger.error(
                f"[背提文本含金量] LogitUpliftProvider 提取 quality tags 失败: {str(e)}",
                exc_info=True,
            )
            return {}
        ...
        if not remarks:
            return {}
        return {
            k: [t for t in v if t not in _INTERNAL_REMARKS]
            for k, v in remarks.items()
            if any(t not in _INTERNAL_REMARKS for t in v)
        }
