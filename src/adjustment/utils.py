...
_SCHOOL_STATS_CACHE: tuple[pd.DataFrame, dict[str, dict[str, float]]] | None = None
...
def compute_school_stats(cases_df: pd.DataFrame) -> dict[str, dict[str, float]]:
    ...
    if _SCHOOL_STATS_CACHE is not None and _SCHOOL_STATS_CACHE[0] is cases_df:
        logger.debug("学校统计: 命中缓存 | n_schools=%d", len(_SCHOOL_STATS_CACHE[1]))
    ...
    _SCHOOL_STATS_CACHE = (cases_df, result)
    logger.info(
