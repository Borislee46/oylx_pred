...
class TextBoostProvider(ABC):
    ...
    def get_quality_tags(self, experience_details: dict[str, str]) -> dict[str, list[str]]:
        return {}
...
class GatedTextBoostProvider(TextBoostProvider):
    ...
    def get_quality_tags(self, experience_details: dict[str, str]) -> dict[str, list[str]]:
        if not has_any_experience(experience_details):
            return {}
        return self._inner.get_quality_tags(experience_details)
...
def _get_text_boost_provider_cached(config_key: str) -> TextBoostProvider:
    ...
    from src.adjustment.logit_uplift.provider import (
        LogitUpliftProvider,
    )
    ...
    max_total_boost = config.get("max_total_boost", 0.15)
