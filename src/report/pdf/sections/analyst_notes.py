...
class _NoopPDFAgent:
    def generate_analyst_notes(self, *args, **kwargs):
        return None


class AnalystNotesGenerator:
    def __init__(self, ai_agent=None):
        self.ai_agent = ai_agent or _NoopPDFAgent()
    ...
    def generate_analyst_notes(self, user_data: dict, soft_skills: dict) -> str:
        ...
        if ai_notes and ai_notes.strip():
            ai_notes = self._sanitize_xml(ai_notes)
    ...
    def _generate_fallback_analyst_notes(self, user_data: dict, soft_skills: dict) -> str:
        ...
        gpa_value = (
            user_data.get("gpa_raw") or user_data.get("gpa") or user_data.get("gpa_score", 0)
        )
