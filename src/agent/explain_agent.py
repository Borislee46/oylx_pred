...
from src.agent.explain_profiles import SYSTEM_PROMPT
...
class ExplainAgent:
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        # 流式状态放 thread-local：get_explain_agent() 是 @lru_cache 单例，若把缓冲写入
        # 实例属性，多会话并发流会在互相覆盖（解释流串流/污染）。每个会话独立线程运行，
        # 用 _stream_local 即可隔离，同时保留单例复用昂贵 pydantic-ai Agent 构建的收益。
        self._stream_local = threading.local()

        self.agent_name = "解释Agent"
        self.logger = _log

    # ---- 流式状态（thread-local：单例复用但按会话线程隔离）----
    @property
    def _stream_buffer(self) -> str:
        return getattr(self._stream_local, "buffer", "")

    @_stream_buffer.setter
    def _stream_buffer(self, value: str) -> None:
        self._stream_local.buffer = value

    @_stream_buffer.deleter
    def _stream_buffer(self) -> None:
        try:
            del self._stream_local.buffer
        except AttributeError:
            pass

    @property
    def _stream_parsed(self) -> Any:
        return getattr(self._stream_local, "parsed", None)

    @_stream_parsed.setter
    def _stream_parsed(self, value: Any) -> None:
        self._stream_local.parsed = value

    @_stream_parsed.deleter
    def _stream_parsed(self) -> None:
        try:
            del self._stream_local.parsed
        except AttributeError:
            pass

    @property
    def _stream_prompt(self) -> str:
        return getattr(self._stream_local, "prompt", "")

    @_stream_prompt.setter
    def _stream_prompt(self, value: str) -> None:
        self._stream_local.prompt = value

    @_stream_prompt.deleter
    def _stream_prompt(self) -> None:
        try:
            del self._stream_local.prompt
        except AttributeError:
            pass

    @property
    def _stream_context(self) -> Any:
        return getattr(self._stream_local, "context", None)

    @_stream_context.setter
    def _stream_context(self, value: Any) -> None:
        self._stream_local.context = value

    @_stream_context.deleter
    def _stream_context(self) -> None:
        try:
            del self._stream_local.context
        except AttributeError:
            pass
