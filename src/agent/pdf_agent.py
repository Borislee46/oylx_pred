...
class PDFAgent(BaseAgent):
    def __init__(self, config: dict[str, Any] | None = None):
        super().__init__(config=config, timeout=15, agent_name="PDFAgent", cache_ttl=86400)
    ...
    def generate_analyst_notes(
        ...
        cache_key = f"analyst_notes:{self._generate_pdf_cache_key(user_data, soft_skills)}"
        ...
        content = self._call_api(prompt, custom_cache_key=cache_key)
