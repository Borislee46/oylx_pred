...
class LeadInDispatcher(_DispatchExecutorMixin):
    ...
    def dispatch(
        ...
        _log.info(
            "DISPATCH | single-agent | len=%d text=%s",
            len(analyze_text),
            analyze_text[:80],
        )
        result = self._dispatch_new(session_manager, ctx, analyze_text, session_state)
        _log.info(
    ...
    def mark_running(self, session_state: dict, analyze_text: str) -> None:
        ...
        if self._sm is not None:
            ...
            cur = self._sm.current
            if cur in (LeadInPhase.BLOCKED, LeadInPhase.PREDICTING):
                self._sm.update(phase=LeadInPhase.IDLE)
                cur = LeadInPhase.IDLE
            if cur != LeadInPhase.EXTRACTING:
