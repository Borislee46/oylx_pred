...
_MAJOR_FACULTY_KEYWORDS_SORTED = sorted(
    ((k.lower(), v) for k, v in _MAJOR_FACULTY_KEYWORDS.items()),
...
def major_to_faculty(major: str) -> str:
    if not major:
        return "未知"
    m = major.lower()
    for keyword, faculty in _MAJOR_FACULTY_KEYWORDS_SORTED:
        if keyword in m:
            return faculty
    return "未知"
...
def compute_bg_school_score(
    ...
    if "background_university" in df.columns:
        df["bg_school_score"] = (
            df["background_university"]
            .astype(str)
            .str.strip()
            .map(school_score_map)
