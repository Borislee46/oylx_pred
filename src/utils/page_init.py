...
from src.utils.page_auth import ensure_login
...
def _sync_auth_to_session(session_mgr: SessionManager) -> None:
    ...
    user_email = st.session_state.get("demo_user_email")
    if user_email:
        contract_tier = _resolve_contract_tier(user_email)
        session_mgr.set(
            user_info={
                "username": user_email,
                "nickname": st.session_state.get("demo_user_nickname", user_email),
    ...
    if st.session_state.get("is_authenticated", False):
        username = st.session_state.get("demo_username", "")
        if username:
            contract_tier = _resolve_contract_tier(username)
            session_mgr.set(
                user_info={
                    "username": username,
                    "nickname": st.session_state.get("demo_nickname", username),
                    "contract_tier": contract_tier,
                },
                is_logged_in=True,
            )
...
def init_page(
    ...
    user_nickname = st.session_state.get("demo_user_nickname", default_nickname)
    user_email = st.session_state.get("demo_user_email", "DEMO_USER_NOT_LOGGED_IN")
