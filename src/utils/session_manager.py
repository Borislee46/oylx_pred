...
class SessionManager:
    ...
    def __init__(self) -> None:
        ...
        existing_st_session_id = st.session_state.get("session_id")
        if existing_st_session_id and not self._model.session_id:
            self._model.session_id = existing_st_session_id
        elif not self._model.session_id:
            self._model.session_id = str(uuid.uuid4())

        if not existing_st_session_id:
            st.session_state["session_id"] = self._model.session_id[:8]
        elif self._model.session_id and existing_st_session_id != self._model.session_id[:8]:
            st.session_state["session_id"] = self._model.session_id[:8]
    ...
    def delete(self, key: str) -> None:
        ...
        elif hasattr(self._model, key) and key != "session_id":
            default_model = UserDataModel()
            default_value = getattr(default_model, key, None)
    ...
    def get_current_user_info(self) -> dict[str, Any]:
        ...
        if st.session_state.get("is_authenticated", False):
            ...
            if username:
                ...
                if stored_user_info.get("username") == username:
