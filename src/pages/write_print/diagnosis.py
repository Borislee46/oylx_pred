...
AI_TRANSITIONS_SET = {
    ...
    "importantly", "significantly", "notably", "subsequently",
    "meanwhile", "accordingly", "alternatively", "besides",
}
...
_STARTER_ALLOWLIST = {'i', 'my', 'we', 'he', 'she', 'they', 'it',
                      'a', 'an', 'and', 'but', 'or', 'so', 'if', 'as',
                      'at', 'in', 'on', 'to', 'for', 'with', 'from',
                      'when', 'while', 'since', 'because', 'although'}
...
def _find_repetitive_starters(sentences: list) -> list:
    ...
    for starter, count in starter_counts.items():
        ...
        if len(words_in) == 1 and count >= 3:
            ...
            issues.append({
                'starter': starter.capitalize(), 'count': count,
                'sentence_indices': positions,
            })
        elif len(words_in) == 3 and count >= 2:
            positions = [i for s, i in starters if s == starter]
            issues.append({
                'starter': ' '.join(w.capitalize() if len(w) > 3 else w
                                     for w in words_in),
                'count': count,
                'sentence_indices': positions,
            })
...
def _find_uniform_runs(sentences: list, sent_lengths: list) -> list:
    ...
    while i < len(sent_lengths) - 2:
        ...
        if len(trio) == 3 and all(l > 8 for l in trio):
            m = statistics.mean(trio)
            if m > 0 and all(abs(l - m) / m < 0.20 for l in trio):
                j = i + 3
                ...
                if j - i >= 3:
                    runs.append({
                        'start': i, 'end': j - 1, 'length': j - i,
                        'avg_words': round(m), 'sentences': sentences[i:j],
                    })
                i = j
                continue
        i += 1
...
def _find_long_sentences(sentences: list, sent_lengths: list,
                         threshold: int = 35) -> list:
    long_ones = []
    for i, (s, wc) in enumerate(zip(sentences, sent_lengths)):
        if wc >= threshold:
            long_ones.append({
                'index': i, 'word_count': wc,
...
def _sentence_rhythm_chart(sent_lengths: list, width: int = 54) -> str:
    ...
    for i, length in enumerate(sent_lengths):
        ...
        marker = ''
        if i >= 2 and all(
            abs(sent_lengths[j] - length) / max(length, 1) < 0.15
...
_DEFAULT_IMPACTS = {
    'keyword': 0.8, 'template': 2.5, 'transition': 1.5,
    'uniform': 3.0, 'long_sent': 1.2, 'starter': 3.0,
}
...
def _verdict(score: float) -> dict:
    if score < 20:
        ...
        return {'label': 'Significant AI patterns',
        ...
        return {'label': 'Strong AI patterns',
