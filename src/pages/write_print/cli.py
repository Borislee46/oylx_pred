...
from src.pages.write_print.diagnosis import _sentence_rhythm_chart, _DIFFICULTY
...
def diagnose(text: str, scaler, model):
    ...
    print()
    print(f"  {ts['words']:,} words  |  {ts['sentences']} sentences  |  "
          f"{ts['paragraphs']} paragraphs")
    print()

    print("  What drives the score:")
    print(f"  {'Factor':<30} {'Weight':>8}  {'Impact'}")
    print(f"  {'-'*30} {'-'*8}  {'-'*6}")
    for name, fb in result['features'].items():
    ...
    if result['global_fixes']:
        ...
        print("\n  No structural issues detected — your rhythm and variety "
              "look good.")
...
def main():
    ...
    if len(sys.argv) >= 3 and sys.argv[1] == '--diagnose':
        ...
        print(f"\nUsage: py score_fitter.py --diagnose <file.pdf|file.txt|"
              f"\"raw text\">")
