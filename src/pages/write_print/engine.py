...
def analyze(text: str, scaler: StandardScaler, model: Ridge) -> dict:
    ...
    for ls in long_sents[:5]:
        _add_fix(
            ...
            f'"{ls["text"]}"',
            "Find a natural break point (after a comma or 'and') and split into 2 sentences.",
            sent_idx=ls["index"],
        )
    ...
    if len(sent_lengths) >= 3:
        scv = (
            statistics.stdev(sent_lengths) / statistics.mean(sent_lengths)
            if statistics.mean(sent_lengths) > 0
            else 0
        )
        short_count = sum(1 for length in sent_lengths if length < 8)
        long_count = sum(1 for length in sent_lengths if length > 30)
        if scv < 0.45:
            msg = (
                f"Your sentences are similar in length throughout "
                f"(only {short_count} very short, {long_count} very long). "
                f"Good writing mixes punchy short sentences with longer ones. "
            )
            if short_count == 0:
                msg += "Try adding a 3-5 word sentence right after a long one — it creates rhythm. "
            if long_count == 0:
                msg += "Pick one key idea and elaborate with detail to stretch it past 30 words. "
