...
def extract_text_from_pdf(pdf_path: str) -> str:
    try:
        ...
        _log.warning(f"PDF extract failed on '{pdf_path}': {e}")
        return ""
...
def _feat_sentence_cv(sent_lengths: list) -> float:
    lengths = [sl for sl in sent_lengths if sl > 1]
    if len(lengths) < 3:
        return 0.5
...
def _feat_paragraph_cv(paragraphs: list) -> float:
    ...
    para_lens = [pl for pl in para_lens if pl > 0]
    if len(para_lens) < 2:
        return 0.5
...
def _compute_features_from_tokens(
    words: list, sent_lengths: list, paragraphs: list, text: str, pos_tags: list = None
) -> np.ndarray:
    feats = [
