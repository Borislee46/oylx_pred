...
def create_student_background_radar_chart(input_data, school_level_scores_map):
    ...
    raw_school_score = school_level_scores_map.get(school_level_str, school_level_scores_map.get("未知", 13))
    ...
    if (max_raw_score_val - min_raw_score_val) != 0:
        val_school = ((max_raw_score_val - raw_school_score) / (max_raw_score_val - min_raw_score_val)) * 100
    else:
        val_school = 50
    ...
    val_academic = min(academic_sum / MAX_ACADEMIC_ITEMS, 1.0) * 100 if MAX_ACADEMIC_ITEMS > 0 else 0
    ...
    val_practical = min(practical_sum / MAX_PRACTICAL_ITEMS, 1.0) * 100 if MAX_PRACTICAL_ITEMS > 0 else 0
