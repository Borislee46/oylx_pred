...
from reportlab.platypus import BaseDocTemplate, PageBreak, PageTemplate, Frame, NextPageTemplate
...
class WatermarkedDocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename, user_nickname: Optional[str] = None, **kwargs):
        ...
        frame_normal = Frame(self.leftMargin, self.bottomMargin, self.width, self.height, id='normal')

        cover_template = PageTemplate(id='CoverPage', frames=[frame_cover])
        later_template = PageTemplate(id='LaterPages', frames=[frame_normal], onPage=self._draw_header_footer_and_watermark)
    ...
    def _draw_header_footer(self):
        ...
        self.canv.line(margin, page_height - 1.5*cm, page_width - self.rightMargin, page_height - 1.5*cm)
    ...
    def _draw_watermark(self):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                margin = 40
                if margin <= x_pos <= page_width - margin and margin <= y_pos <= page_height - margin:
                    self.canv.saveState()
...
class PDFReportGenerator:
    ...
    def generate_report(self, 
                       ...
                       user_nickname: str,
                       optimization_results: Optional[Dict] = None) -> bytes:
        try:
            buffer = io.BytesIO()
            from ..config import pdf_config
            ...
            story.extend(self.section_builder.create_cover_page(final_nickname, "个性化申请方案报告"))
            ...
            if optimization_results:
                story.extend(self.section_builder.create_optimization_strategies_section(optimization_results, cases_df))
