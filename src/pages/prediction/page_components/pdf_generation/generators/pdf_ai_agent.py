...
class PDFAIAgent:
    ...
    def _truncate_if_needed(self, content: str, max_length: int = 250) -> str:
        ...
        if truncated_lines:
            ...
            if not header_present and "<b>分析师建议:</b>" not in result:
                result = "<b>分析师建议:</b><br/>" + result if result.strip() else "<b>分析师建议:</b>"
            return result
    ...
    def _generate_cache_key(self, user_data: Dict[str, Any], soft_skills: Dict[str, Any]) -> str:
        key_data = {
            "user_data": {
                ...
                if k in ("gpa_score", "gpa_raw", "gpa_scale", "background_university", "background_major", "language_type", "language_score", "language_score_raw")
            },
    ...
    def generate_analyst_notes(
        ...
        logger.info(f"PDFAIAgent: 构建prompt完成，长度: {prompt_length}，模型: {self.model}，API URL: {self.api_url}")
        ...
        try:
            ...
            logger.info(f"PDFAIAgent: API请求完成，状态码: {response.status_code}，耗时: {elapsed_time:.2f}秒")
            ...
            logger.warning(f"PDFAIAgent: API返回错误: {error_message}，响应: {json.dumps(response_json, ensure_ascii=False)[:200]}")
            return None

        except requests.exceptions.Timeout:
            elapsed_time = time.time() - start_time
            logger.warning(f"PDFAIAgent: 请求超时（{self.timeout}秒），实际耗时: {elapsed_time:.2f}秒，使用fallback建议")
            return None
        except requests.exceptions.RequestException as e:
            elapsed_time = time.time() - start_time
            logger.warning(f"PDFAIAgent: API调用失败: {e}，耗时: {elapsed_time:.2f}秒，使用fallback建议")
            return None
        except Exception as e:
            elapsed_time = time.time() - start_time
            logger.warning(f"PDFAIAgent: 生成分析师建议时发生错误: {e}，耗时: {elapsed_time:.2f}秒，使用fallback建议", exc_info=True)
            return None
