...
page_data_loader_logger = setup_logger("page3", "prediction")


def get_prediction_model(model_name):
    global_categories_df_instance = load_global_categories_dataframe()
    try:
        return PredictionModel(model_name, global_categories_df=global_categories_df_instance)
    except ValueError as exc:
        page_data_loader_logger.error(f"预测模型加载失败: {exc}")
        return None
