...
def _execute_prediction_pipeline(
    ...
    prediction_handler_logger.info(
        "Pipe 启动 | model=%s targets=%d features=%d fingerprint=%s cache=%s",
        model_name,
    ...
    if meta and meta.get("fallback_eligible") and meta.get("_fallback_combinations"):
        ...
        if fallback_results:
            ...
            if _fb_text_provider is not None:
                try:
                    from src.agent import get_text_preprocessing_agent

                    _fb_qa = get_text_preprocessing_agent()
    ...
    if _text_provider is not None:
        try:
            ...
            _qa = get_text_preprocessing_agent()
