...
def _attach_metadata(results: list, background_major: str, similarity_cache: dict) -> list:
    if not results: return []
    ...
    sims = get_cached_major_similarities_batch(pairs, cache=similarity_cache) if bg_major else [0.0]*len(results)
    ...
    for i, r in enumerate(results):
        ...
        raw_sim = sims[i]
        r["similarity"] = adjust_similarity_score(background_major=bg_major, target_major=str(m), similarity=raw_sim) if bg_major else 0.0
...
def process_prediction_results(
    ...
    results = _attach_metadata(results, background_major, bg_target_similarity_cache)
