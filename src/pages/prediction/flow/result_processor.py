...
from src.pages.prediction.core.utils import cached_token_sort_ratio, get_cached_major_similarity
...
class SingleResultProcessor:
    ...
    def process(self, result: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if self.bg_orig_lower:
            score_en = cached_token_sort_ratio(self.bg_orig_lower, m_lower)
            score_cn = (
                cached_token_sort_ratio(self.bg_orig_lower, res["major_cn"].lower())
                if res["major_cn"]
                else 0
            )
