...
def load_raw_cases_data(
    path: str = "src/ml/data/cases.feather",
):
    import numpy as np

    df = pd.read_feather(_resolve_path(path))
...
from src.utils.schools.data import load_school_base_data  # noqa: F401


@st.cache_data(show_spinner=False)
def load_school_major_details_df(
    path="src/ml/data/school_major_details.feather",
):
    return pd.read_feather(_resolve_path(path))
