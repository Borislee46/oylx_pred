...
def load_model_dependencies(
    ...
    if selected_model_path is None:
        ...
        for ext in [".ubj.gz", ".ubj"]:
            model_glob_pattern = f"{model_prefix}_????????_??????{ext}"
        ...
        if not model_files:
            ...
            return None, None, None, None, 0.24, None
    ...
    cross_level_levels = _safe_json_loads(booster.attr("cross_level_levels"), "cross_level_levels")
    if cross_level_levels is None:
        cross_level_levels = metadata.get("cross_level_levels")
    if cross_level_levels is not None and not isinstance(cross_level_levels, list):
        cross_level_levels = None
...
def load_model(
    ...
    (
        ...
        prediction_threshold,
        cross_level_levels,
    ) = load_model_dependencies("src/ml/pre-trained_models", model_name)
