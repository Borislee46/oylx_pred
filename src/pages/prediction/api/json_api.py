...
def predict(payload: dict[str, Any], confirm_cross_faculty: bool = False) -> dict[str, Any]:
    try:
        ...
        task = TaskResult(task_id=task_id, status="running", progress=10)
        _TASK_STORE[task_id] = task

        result = _execute_prediction_core(payload, v["normalized_input"], confirm_cross_faculty)
...
def _validate_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return validate_and_normalize(payload, cases_df=cases_df, school_base_df=school_base_df)
...
def get_prediction_status(task_id: str) -> dict[str, Any]:
