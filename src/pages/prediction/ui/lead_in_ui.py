...
def _render_persisted(session_manager: Any) -> None:
    ...
    _render_trace(state if has_sm else None)


def _render_trace(state: Any) -> None:
    """渲染这一轮的工具调用过程，供完成回看。

    与实时步骤日志**共用同一个渲染器（`StepLog`）**，所以措辞与排版只有一处定义。
    只在回看这一侧多做一件事：把它折叠起来，并标出失败步数。

    为什么用原生 `st.status` 而不是自定义组件：回看发生在 dispatch 返回之后、
    运行在脚本线程上，原生元素最省事（主题、可访问性、复制全都免费，且不用前端构建）。
    """
    import streamlit as st
    ...
    # 同一轮里实时步骤日志已经展示过 → 不要再渲染一遍（避免两份步骤）。
    # 这个标记只在"实时路径刚跑完、还在同一次脚本运行里"存在；
    # 页面重跑后它已被消费，此时实时日志也不在了，就由这里重建回看视图。
    if st.session_state.pop("_lead_in_live_trace_shown", False):
        return

    entries = list(getattr(state, "last_trace", None) or []) if state is not None else []
    lines = entries_to_detail_lines(entries, limit=20)
    if not lines:
        return
    ...
    def _ok(entry: Any) -> bool:
        ...
        return bool(getattr(entry, "ok", True))

    failed = sum(1 for e in entries if not _ok(e))
    title = f"AI 的处理过程 · {len(lines)} 步"
    if failed:
        title += f"（{failed} 步失败）"

    log = StepLog(title=title, expanded=False)
    log.draw(lines)
    log.raw_details(entries)
    log.complete(title)
