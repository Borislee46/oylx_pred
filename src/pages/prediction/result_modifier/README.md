...
- **语言惩罚**：低于 pass_line 离散阶梯惩罚（L1/L2/L3/L3.5），基于历史 `mean - 0.5*std` 为 pass_line。2026-05 新增 L3.5（0.20），将 6.0-6.5 雅思密集区拆分为两档，区分 6.0（L3, 0.40）和 6.3+（L3.5, 0.20）。用户行为分析显示 89% 的提交集中在 6.0-6.5，此改动让该区间从 1 个 bucket 变为 2 个，提升区分度
