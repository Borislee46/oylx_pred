...
def _generate_university_options(session_manager, cases_df):
    ...
    if school_base_df is not None and "学校名称" in school_base_df.columns:
        universities.update(
            v
            for v in school_base_df["学校名称"].dropna().astype(str).unique()
...
def _generate_major_options(cases_df):
    ...
    majors_display = [
        m
        for m in mapping_df["background_major_original"].dropna().astype(str).unique()
        if m.strip().lower() not in ("", "none", "nan")
    ]
