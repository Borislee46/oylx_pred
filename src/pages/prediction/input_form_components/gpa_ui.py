from functools import partial

import streamlit as st

from src.pages.prediction.handler_config import DEFAULT_FORM_KEYS, DEFAULT_WIDGET_KEYS
from src.pages.prediction.input_form_components.form_config import (
    GPA_SCALES,
    GPA_WARNING_THRESHOLDS,
)


def _check_and_show_gpa_warning(session_manager):
    gpa_raw_input = session_manager.get(DEFAULT_FORM_KEYS.gpa_raw_input)
    gpa_scale = session_manager.get(DEFAULT_FORM_KEYS.gpa_scale)
    if (
        gpa_raw_input is not None
        and gpa_raw_input > 0
        and gpa_raw_input < GPA_WARNING_THRESHOLDS[gpa_scale]
    ):
        threshold = GPA_WARNING_THRESHOLDS[gpa_scale]
        warning_key = f"gpa_warning_{gpa_raw_input:.2f}_{gpa_scale}"
        if session_manager.get(DEFAULT_FORM_KEYS.last_gpa_warning_key) != warning_key:
            st.toast(
                f"注意：当前GPA {gpa_raw_input:.2f}（{gpa_scale}制）低于提示线 {threshold:g}，预测结果可能会明显下调"
            )
            session_manager.set(last_gpa_warning_key=warning_key)


def render_gpa_section(session_manager, form_state_manager, logger):
    current_scale = session_manager.get(DEFAULT_FORM_KEYS.gpa_scale)
    if current_scale not in GPA_SCALES:
        current_scale = list(GPA_SCALES.keys())[0]
        session_manager.set(gpa_scale=current_scale)

    if (
        DEFAULT_WIDGET_KEYS.gpa_scale not in st.session_state
        or st.session_state.get(DEFAULT_WIDGET_KEYS.gpa_scale) not in GPA_SCALES
    ):
        st.session_state[DEFAULT_WIDGET_KEYS.gpa_scale] = current_scale

    st.segmented_control(
        "GPA 分制",
        options=list(GPA_SCALES.keys()),
        selection_mode="single",
        on_change=partial(form_state_manager.gpa_scale_changed, session_manager),
        key=DEFAULT_WIDGET_KEYS.gpa_scale,
    )

    current_gpa_scale_details = GPA_SCALES[session_manager.get(DEFAULT_FORM_KEYS.gpa_scale)]
    max_val = float(current_gpa_scale_details["max"])
    if DEFAULT_WIDGET_KEYS.gpa_raw_input not in st.session_state:
        default_gpa_value = session_manager.get(DEFAULT_FORM_KEYS.gpa_raw_input)
        st.session_state[DEFAULT_WIDGET_KEYS.gpa_raw_input] = (
            default_gpa_value if default_gpa_value is not None else 3.0
        )
    else:
        cur_val = st.session_state.get(DEFAULT_WIDGET_KEYS.gpa_raw_input)
        if cur_val is None:
            st.session_state[DEFAULT_WIDGET_KEYS.gpa_raw_input] = 3.0
        elif isinstance(cur_val, (int, float)) and cur_val > max_val:
            st.session_state[DEFAULT_WIDGET_KEYS.gpa_raw_input] = max_val
    gpa_raw = st.number_input(
        f"GPA (满分 {session_manager.get(DEFAULT_FORM_KEYS.gpa_scale)})",
        min_value=0.0,
        max_value=max_val,
        step=float(current_gpa_scale_details["step"]),
        format=current_gpa_scale_details["format"],
        on_change=lambda: (
            session_manager.set(
                gpa_raw_input=session_manager.get_widget_value(DEFAULT_WIDGET_KEYS.gpa_raw_input)
            ),
            form_state_manager.on_form_change(session_manager, change_type="text"),
        ),
        placeholder="",
        key=DEFAULT_WIDGET_KEYS.gpa_raw_input,
    )
    if gpa_raw != session_manager.get(DEFAULT_FORM_KEYS.gpa_raw_input):
        session_manager.set(gpa_raw_input=gpa_raw)

    if session_manager.get(DEFAULT_FORM_KEYS.gpa_raw_input) is not None:
        _check_and_show_gpa_warning(session_manager)

    return gpa_raw
