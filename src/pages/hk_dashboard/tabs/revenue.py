...
def render(data: dict[str, pd.DataFrame]) -> None:
    ...
    if proj_filter: filtered = filtered[filtered["产品品类"] == proj_filter]
    if biz_filter:  filtered = filtered[filtered["业务类型"] == biz_filter]
    ...
    st.html('<div class="hk-note">收入人次 | GROUP BY MONTH(业务日期) | SUM(现金收入) | 退费含负值</div>')

    with st.expander("收入明细"):
        detail = (
            filtered.assign(_amt=pd.to_numeric(filtered["现金收入"], errors="coerce"))
            .groupby(["产品品类", "科目", "季度"], as_index=False)["_amt"].sum()
            .rename(columns={"_amt": "现金收入"}).sort_values("现金收入", ascending=False)
        )
    ...
    with c1:
        ...
        monthly_trend_line(def_monthly, "月份", "结转收入", color="#d97706", currency=True, height=200)
    ...
    with c2:
        ...
        if not tch.empty:
            simple_bar(tch.head(10), "主带课教师", "结转收入", color="#d97706", horizontal=True, fmt=",.0f", height=200)
