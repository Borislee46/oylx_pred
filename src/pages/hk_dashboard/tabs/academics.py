...
def _teacher_workload(class_master: pd.DataFrame) -> pd.DataFrame:
    """Compute per-teacher teaching hours and class count."""
...
def render(data: dict[str, pd.DataFrame]) -> None:
    ...
    render_metric_grid([
        ...
        {"label": "在读 (有效)", "value": str(valid_count)},
        {"label": "离班 (无效)", "value": str(invalid_count)},
    ], columns=3)
    ...
    with col2:
        ...
        if "学员年级(自动更新)" in r.columns:
            ...
            if not grade_cnt.empty:
                simple_bar(grade_cnt.head(10), "年级", "count", horizontal=True, color="#73c0de")
    ...
    with col1:
        top_t = workload.head(15)
        if not top_t.empty:
            simple_bar(top_t, "教师", "课时", horizontal=True, color="#008a6c")
        else:
            st.caption("暂无教师课量数据")
    with col2:
