...
from src.pages.hk_dashboard.metrics.funnel_metrics import (
    consultant_ranking, class_capacity_metrics, channel_breakdown, tmk_processing_stats,
)
...
def _render_pipeline(kehu, tmk, qianyue) -> None:
    ...
    with c1:
        ...
        stages = [
            ...
            ("已外呼", tmk["外呼次数"].notna().sum(), "#0d9488"),
            ("有工单", tmk["工单ID"].notna().sum(), "#d97706"),
            ("工单正常", (tmk["工单状态"] == "正常").sum(), "#7c3aed"),
        ]
        ...
        for i, (label, val, color) in enumerate(stages):
            ...
            html += (
                ...
                f'<span style="width:50px;font-size:0.72rem;font-weight:600;color:#475569">{label}</span>'
                f'<span style="width:48px;font-size:0.76rem;font-weight:700;color:{color}">{val:,}</span>'
                f'<span style="width:30px;font-size:0.64rem;color:#94a3b8">{ratio}</span>'
                f'<span style="flex:1;margin-left:3px;background:{color};height:14px;'
                f'border-radius:2px;width:{max(pct * 0.4, 1)}%"></span>'
                f'</div>'
            )
        html += "</div>"
        st.html(html)
        st.html('<div class="hk-note">客服TMK资源处理 | 资源ID → 外呼次数 → 工单ID → 工单状态</div>')
    ...
    with c2:
        ...
        for label, val, color in items:
            ...
            html2 += (
                f'<div style="margin-bottom:3px;display:flex;align-items:center">'
                f'<span style="width:55px;font-size:0.72rem;font-weight:600;color:#475569">{label}</span>'
                f'<span style="width:48px;font-size:0.76rem;font-weight:700;color:{color}">{val:,}</span>'
                f'<span style="flex:1;margin-left:3px;background:{color};height:14px;'
                f'border-radius:2px;width:{max(pct * 0.4, 1)}%"></span>'
                f'</div>'
        ...
        st.caption(f"{total_signed} 单签约 | {matched}/{len(signed_ids)} ({matched_pct}) 资源 ID 可匹配到资源池")
    ...
    st.html('<div class="hk-note">TMK | 分配时间 → 顾问处理时间 = 处理时延 | COUNT(外呼次数 IS NOT NULL) = 已外呼</div>')
...
def render(data: dict[str, pd.DataFrame]) -> None:
    ...
    st.html('<div class="hk-note">维表 | 正常 + 当前人数>0 | 满班率 = 满班数 / 行课班级数 | 满班 = 当前人数 >= 标准人数</div>')
    ...
    with c1:
        ...
        if not ch.empty: donut_chart(ch, "渠道", "count", max_categories=6, height=220)
    with c2:
        st.html("<h3>签约渠道</h3>")
        if "二级获取渠道" in qianyue.columns:
            ...
            if not ch2.empty:
                simple_bar(ch2.head(10), "渠道", "count", horizontal=True, color="#0d9488", height=200)
