...
from src.pages.hk_dashboard.components.kpi_cards import render_metric_grid, render_kpi_row
...
def render(data: dict[str, pd.DataFrame]) -> None:
    ...
    if not renewal_df.empty:
        ...
        overall_rate = 0; total_bonus = 0; total_students = 0; retained = 0

    render_kpi_row([
        {"value": f"{overall_rate:.0%}", "label": "整体续费率", "accent": "positive"},
        {"value": str(total_students), "label": "当月学员总数", "accent": "accent"},
        {"value": str(retained), "label": "次月在班数", "accent": "accent"},
        {"value": f"HK$ {total_bonus / 1e4:.0f}万", "label": "应发奖金", "accent": "warning"},
    ])

    # ── Teacher ranking + bonus table ──
    st.html("<h3>教师续费率排名</h3>")
    c1, c2 = st.columns([2, 3])
    with c1:
        if not renewal_df.empty:
            r_display = renewal_df.copy()
            r_display["续费率_pct"] = (r_display["续费率"] * 100).round(1)
            simple_bar(r_display.head(15), "教师", "续费率", horizontal=True, color="#2563eb")
    with c2:
    ...
    if not renewal_df.empty:
        ...
        simple_bar(dist, "区间", "count", color="#059669")
