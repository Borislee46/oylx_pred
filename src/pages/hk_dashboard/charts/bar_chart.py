...
def simple_bar(
    ...
    sort_desc: bool = True, fmt: str = ",.0f",
) -> None:
    ...
    if horizontal:
        ...
        y = alt.Y(f"{y_col}:Q", title=None,
                  axis=alt.Axis(grid=True, gridColor="#f1f5f9", labelFontSize=11, tickCount=4, format=fmt))

    chart = (
        alt.Chart(df)
        .mark_bar(color=color, cornerRadiusTopLeft=3, cornerRadiusTopRight=3,
                  size=22 if not horizontal else 14)
        .encode(x=x, y=y, tooltip=[alt.Tooltip(f"{x_col}:N"),
...
def grouped_bar(
    ...
    chart = (
        ...
        .encode(
            ...
            y=alt.Y(f"{y_col}:Q", title=None,
                    axis=alt.Axis(grid=True, gridColor="#f1f5f9", labelFontSize=11, tickCount=4, format=",.0f")),
