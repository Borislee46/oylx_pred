...
**标准报告短语示例：**
> “Shapiro-Wilk 检验表明，A 组数据偏离正态分布假设（$W = 0.85, p < .001$），且正态 Q-Q 图显示尾部明显偏离参考线。基于中位数的 Levene 检验（Brown-Forsythe test）表明，各组方差齐性假设成立（$F(2, 87) = 1.23, p = .297$）。由于数据存在显著偏态，后续组间比较采用非参数 Kruskal-Wallis 检验，描述性统计采用中位数与四分位距（IQR）报告。”
