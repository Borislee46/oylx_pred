...
- **缺失值处理（列表删除陷阱）**：SPSS 默认使用「列表删除（Listwise Deletion）」，只要被试在任一题项漏答，整条数据即被剔除。题量大时会导致有效样本量（$N$）锐减，甚至改变样本代表性。建议在分析前进行缺失值处理（首选**全信息最大似然 FIML** 或**多重插补 MI**，避免均值插补引入偏差）或仔细核查有效 $N$ 数。
