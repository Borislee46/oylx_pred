...
- **组间秩检验**：均为加权对数秩统计量。
  - **Log-rank**：权重 \(w(t) = 1\)，对各时间点事件赋予**同等权重**，在比例风险（PH）假设下检验效能最优；Breslow/Wilcoxon（权重正比于风险人数）才偏早期差异。
  - **Breslow (Generalized Wilcoxon)**：权重正比于风险人数 \(n_i\)，**对早期差异更敏感**（早期风险人数多 → 权重大；后期风险人数衰减 → 权重小）。
...
- **预测模型评价**：若用于构建列线图（Nomogram）等预测工具，需报告：
  - **区分度 (Discrimination)**：**Harrell's C-index**（全局一致区分度，不等同于时依 AUC；如需时间动态区分度，应报告 **Uno's C** 或 **时依 ROC/AUC**）。
...
- **限制平均生存时间（RMST）**：当比例风险假设不成立时，RMST 是一种无需 PH 假设的稳健替代方案。RMST 比较截至特定时间点 $\tau$ 的**生存曲线下面积差**，解释直观（如”5 年限制平均生存时间差为 X 个月”），近年临床试验中应用快速增长。
