...
def run_multi_obj_optimization_test():
    ...
    with exp_config:
        ...
        with col1:
            ...
            problem_group = st.selectbox("问题系列", ["ZDT (2目标)", "DTLZ (多目标)", "WFG (复杂前沿)"])
            ...
            if problem_group == "ZDT (2目标)":
                ...
                problem_name = st.selectbox("测试问题", ["wfg1", "wfg2", "wfg3", "wfg4", "wfg5", "wfg6", "wfg7", "wfg8", "wfg9"])
        ...
        with col2:
            ...
            if mode == "单算法分析":
                ...
                algo_choices = st.multiselect("选择对比算法", algo_list, default=["NSGA-II", "NSGA-III"])
    ...
    problem = get_problem(problem_name, n_obj=n_obj) if ("dtlz" in problem_name or "wfg" in problem_name) else get_problem(problem_name)
    ...
    def get_algo(choice):
        if choice == "NSGA-II":
            ...
            return NSGA3(pop_size=pop_size, ref_dirs=ref_dirs, crossover=crossover, mutation=mutation)
        elif choice == "MOEA/D":
            ...
            ref_dirs = get_reference_directions("energy", n_obj, n_points=pop_size)
            return RVEA(pop_size=pop_size, ref_dirs=ref_dirs, crossover=crossover, mutation=mutation)
        return None
    ...
    if st.button("开始优化实验"):
        ...
        if n_obj == 2:
            ...
            for (name, data), color in zip(all_res.items(), colors):
                f_data = data["res"].F
                if f_data is not None:
                    ax.scatter(f_data[:, 0], f_data[:, 1], f_data[:, 2], color=color, s=30, label=name)
            ax.set_xlabel("$f_1$")
            ax.set_ylabel("$f_2$")
            ax.set_zlabel("$f_3$")
        ...
        if metrics:
            ...
            export_df = pd.DataFrame(all_res[selected_export]["res"].F, columns=[f"f{i+1}" for i in range(n_obj)])
            csv = export_df.to_csv(index=False).encode('utf-8')
            st.download_button("下载选定算法的解集 (CSV)", csv, f"{selected_export}_results.csv", "text/csv")
        else:
            st.error("未能找到任何有效解。")
