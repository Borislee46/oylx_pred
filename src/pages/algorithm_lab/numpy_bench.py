...
def run_numpy_performance_test():
    ...
    with tabs[0]:
        ...
        test_size = st.select_slider(
            "数据规模 (N)", options=[1, 10, 100, 1000, 10000, 100000], value=1000, key="exp_n"
        )
    ...
    with tabs[1]:
        ...
        init_size = st.select_slider(
            "数组大小", options=[10**3, 10**4, 10**5, 10**6, 10**7], value=10**5, key="init_n"
        )
        fill_val = 3.14
    ...
    with tabs[2]:
        ...
        sort_size = st.select_slider(
            "数据大小", options=[10**3, 10**4, 10**5, 10**6], value=10**4, key="sort_n"
        )
        ...
        if st.button("运行排序对比", key="btn_sort"):
            ...
            # np.partition (Top-K)
            b_nppart = bench(
                lambda: np.partition(data, k_val)[:k_val], warmup=warmup, repeat=repeat
            )
    ...
    with tabs[3]:
        ...
        cw_size = st.select_slider(
            "数据规模", options=[10**3, 10**4, 10**5, 10**6], value=10**5, key="cw_n"
        )
    ...
    with tabs[4]:
        ...
        agg_size = st.select_slider(
            "数据大小 ", options=[10**4, 10**5, 10**6, 10**7], value=10**6, key="agg_n"
        )
    ...
    with tabs[5]:
        ...
        u_size = st.select_slider(
            "去重规模", options=[10**3, 10**4, 10**5, 10**6], value=10**5, key="u_n"
        )
        ...
        if st.button("运行去重对比", key="btn_np_u"):
            ...
            st.info(
                "注：np.unique 内部通常涉及排序，而 Python set 使用哈希表，在某些情况下 set 可能更快，但 np.unique 返回的是有序数组。"
            )
