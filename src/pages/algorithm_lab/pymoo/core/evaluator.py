...
class Evaluator:
    ...
    def eval(
        ...
        **kwargs,
        ...
        evaluate_values_of = (
            self.evaluate_values_of if evaluate_values_of is None else evaluate_values_of
        )
        ...
        if val is not None:
            for individual in pop:
                if len(individual.evaluated) == 0:
                    ...
                    individual.G = (
                        np.full(problem.n_ieq_constr, val) if problem.n_ieq_constr > 0 else None
                    )
                    individual.H = (
                        np.full(problem.n_eq_constr, val) if problem.n_eq_constr else None
                    )
