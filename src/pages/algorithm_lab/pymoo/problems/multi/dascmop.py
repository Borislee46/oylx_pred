...
class DASCMOP(Problem):
    def __init__(self, n_obj, n_ieq_constr, difficulty, **kwargs):
        super().__init__(
            n_var=30, n_obj=n_obj, n_ieq_constr=n_ieq_constr, vtype=float, xl=0.0, xu=1.0, **kwargs
        )
    ...
    def g3(self, X):
        ...
        contrib = (
            X[:, self.n_obj - 1 :] - np.cos(0.25 * j / self.n_var * np.pi * (X[:, 0:1] + X[:, 1:2]))
        ) ** 2
        return contrib.sum(axis=1)[:, None]

    def _calc_pareto_front(self, *args, **kwargs):
        return Remote.get_instance().load(
            "pymoo", "pf", "DASCMOP", f"{str(self.__class__.__name__).lower()}_{self.difficulty}.pf"
        )
