...
class G9(G):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        g4 = 2 * v1 + v2 - 3 * x[:, 0] * x[:, 1] + 2 * x[:, 2] ** 2 + 5.0 * x[:, 5] - 11 * x[:, 6]
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for i in range(1, 10):
            for j in range(1, 10):
                for k in range(1, 10):
                    g = anp.minimum(
                        g, (x[:, 0] - i) ** 2 + (x[:, 1] - j) ** 2 + (x[:, 2] - k) ** 2 - 0.0625
                    )
...
class G13(G):
    ...
    def _calc_pareto_set(self):
        opt = np.array(
            [-1.7171435947203, 1.5957097321519, 1.8272456947885, -0.7636422812896, -0.7636439027742]
        )
        ps = [
            opt,
...
class G20(G):
    def __init__(self):
        ...
        xu = np.full(n_var, 10.0)
        super().__init__(
            n_var=n_var, n_obj=1, n_ieq_constr=6, n_eq_constr=14, xl=xl, xu=xu, vtype=float
        )
