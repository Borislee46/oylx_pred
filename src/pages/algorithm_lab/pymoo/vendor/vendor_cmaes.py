...
from cma.evolution_strategy import CMAEvolutionStrategy, cma_default_options
        ...
        while True:  # restart loop
            ...
            if not bipop:
                ...
                opts["popsize"] = np.floor(
                    popsize0 * popsize_multiplier ** (random_state.uniform() ** 2)
                )
            ...
            if irun == 0 and isinstance(x0, MyCMAEvolutionStrategy):
                ...
                if irun and eval(str(fmin_options["restart_from_best"])):
                    utils.print_warning(
                        "CAVE: restart_from_best is often not useful", verbose=opts["verbose"]
                    )
            ...
            if 1 < 3:
                while not es.stop():  # iteration loop
                    ...
                    X, fit = yield from es.ask_and_eval(
            ...
            if (
                irun - runs_with_small > fmin_opts["restarts"]
