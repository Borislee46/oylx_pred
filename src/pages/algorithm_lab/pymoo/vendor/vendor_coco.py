...
class COCOProblem(Problem):
    def __init__(self, name, n_var=10, pf_from_file=True, **kwargs):
        ...
        coco = self.object
        n_var, n_obj, n_ieq_constr = (
            coco.number_of_variables,
            coco.number_of_objectives,
            coco.number_of_constraints,
        )
        xl, xu = coco.lower_bounds, coco.upper_bounds

        super().__init__(
            n_var=n_var, n_obj=n_obj, n_ieq_constr=n_ieq_constr, xl=xl, xu=xu, **kwargs
        )
    ...
    def _calc_pareto_front(self, *args, **kwargs):
        if self.pf_from_file:
            return Remote.get_instance().load("pymoo", "pf", "bbob.pf", to="json")[
                str(self.function)
            ][str(self.instance)]
        else:
            ps = self.pareto_set()
...
def get_bbob(name, n_var=10, **kwargs):
    ...
    assert (
        1 <= n_function <= 24
    ), f"BBOB has 24 different functions to be chosen. {n_function} is out of range."

    suite_filter_options = (
        f"function_indices: {n_function} instance_indices: {n_instance} dimensions: {n_var}"
    )
