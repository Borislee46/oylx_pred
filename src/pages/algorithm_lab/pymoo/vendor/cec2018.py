...
#       f:      objective vector
#
from numpy import exp, power, prod, setdiff1d
...
def cec2018_DF(problemID="DF1", x=None, t=None):
    ...
    if problemID == "DF13":
        ...
        f[2] = (
            g * sin(0.5 * pi * x[0]) ** 2
            + sin(0.5 * pi * x[0]) * cos(p * pi * x[0]) ** 2
            + sin(0.5 * pi * x[1]) ** 2
            + sin(0.5 * pi * x[1]) * cos(p * pi * x[1]) ** 2
        )
    if problemID == "DF14":
        G = sin(0.5 * pi * t)
        g = 1 + sum((x[2:] - G) ** 2)
...
import numpy as np
from numpy import arange, cos, dot, floor, multiply, pi, sin
...
def cec2018_DF_PF(probID=None, t=1, n_points=100, *args, **kwargs):
    ...
    if "DF10" == (probID):
        ...
        f2 = multiply(dot(g, sin(dot(dot(0.5, pi), x2)) ** H), cos(dot(dot(0.5, pi), x1)) ** H)
        f3 = multiply(dot(g, cos(dot(dot(0.5, pi), x2)) ** H), cos(dot(dot(0.5, pi), x1)) ** H)
    ...
    if "DF12" == (probID):
        ...
        tmp2 = abs(
            multiply(
                sin(dot(floor(dot(k, (dot(2, x1) - 1))), pi) / 2),
                sin(dot(floor(dot(k, (dot(2, x2) - 1))), pi) / 2),
            )
        )
        g = 1 + tmp2
        f1 = multiply(multiply(g, cos(dot(dot(0.5, pi), x2))), cos(dot(dot(0.5, pi), x1)))
        f2 = multiply(multiply(g, sin(dot(dot(0.5, pi), x2))), cos(dot(dot(0.5, pi), x1)))
    ...
    if "DF13" == (probID):
        ...
        f3 = (
            multiply(g, sin(dot(dot(0.5, pi), x1)) ** 2)
            + multiply(sin(dot(dot(0.5, pi), x1)), cos(dot(dot(p, pi), x1)) ** 2)
            + sin(dot(dot(0.5, pi), x2)) ** 2
            + multiply(sin(dot(dot(0.5, pi), x2)), cos(dot(dot(p, pi), x2)) ** 2)
        )
        h = get_PF(np.array([f1, f2, f3]), true)
    if "DF14" == (probID):
        x1, x2 = np.meshgrid(np.linspace(0, 1, H), np.linspace(0, 1, H), indexing="xy")
        ...
        f2 = multiply(
            multiply(g, (1 - x2 + dot(0.05, sin(dot(dot(6, pi), x2))))),
