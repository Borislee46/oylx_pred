...
from src.pages.algorithm_lab.pymoo.algorithms.soo.nonconvex.es import es_mut_repair, es_sigma
from src.pages.algorithm_lab.pymoo.algorithms.soo.nonconvex.sres import SRES
...
class ISRES(SRES):
    ...
    def _setup(self, problem, **kwargs):
        ...
        chi = 1 / (2 * n) + 1 / (2 * (n**0.5))
        varphi = sqrt(
            (2 / chi) * log((1 / self.alpha) * (exp(self.phi**2 * chi / 2) - (1 - self.alpha)))
        )
