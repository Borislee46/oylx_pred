...
from src.pages.algorithm_lab.pymoo.core.algorithm import LoopwiseAlgorithm
...
from src.pages.algorithm_lab.pymoo.core.variable import Integer, Real, get
from src.pages.algorithm_lab.pymoo.docs import parse_doc_string
from src.pages.algorithm_lab.pymoo.operators.crossover.pcx import PCX
...
class G3PCX(LoopwiseAlgorithm):
    ...
    def _initialize_infill(self):
        return self.initialization.do(
            self.problem, get(self.pop_size), algorithm=self, random_state=self.random_state
        )
    ...
    def _next(self, **kwargs):
        ...
        for _ in range(loops_per_iter):
            ...
            S[:, 0] = 0
            fast_fill_random(
                S, len(self.pop), columns=range(1, n_parents), random_state=self.random_state
            )

            off = self.crossover(
                self.problem, self.pop, parents=S, algorithm=self, random_state=self.random_state
            )
