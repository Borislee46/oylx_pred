...
from src.pages.algorithm_lab.pymoo.algorithms.soo.nonconvex.es import ES
from src.pages.algorithm_lab.pymoo.algorithms.soo.nonconvex.ga import GA
from src.pages.algorithm_lab.pymoo.core.problem import Problem
from src.pages.algorithm_lab.pymoo.optimize import minimize as moomin
...
def create_vf_scipy_poly(P, ranks, delta, eps_max, method="trust-constr", verbose=False):
    ...
    res = scimin(_obj_func, x0, constraints=constr, bounds=bounds, method=method, hess=hess)
...
class OptimizeLinearVF(Problem):
    def __init__(self, P, ranks, delta, eps_max, vf):
        ...
        super().__init__(
            n_var_vf, n_obj=1, n_ieq_constr=n_ieq_c_vf, n_eq_constr=1, xl=xl_vf, xu=xu_vf
        )
...
class OptimizePolyVF(Problem):
    def __init__(self, P, ranks, delta, eps_max, vf):
        ...
        super().__init__(
            n_var_vf, n_obj=1, n_ieq_constr=n_ieq_c_vf, n_eq_constr=M, xl=xl_vf, xu=xu_vf
        )
