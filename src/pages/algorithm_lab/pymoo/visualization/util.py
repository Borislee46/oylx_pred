...
def normalize(data, bounds, reverse=False, return_bounds=False):
    from src.pages.algorithm_lab.pymoo.util.normalization import normalize as _normalize
...
def animate(path_to_file, H, problem=None, func_iter=None, plot_min=None, plot_max=None):
    ...
    if problem is not None:
        ...
        plt.scatter(
            pf[:, 0], pf[:, 1], label="Pareto Front", s=60, facecolors="none", edgecolors="r"
        )
...
def plot_problem_surface(
    problem, n_samples, plot_type="wireframe", cmap="summer", show=True, return_figure=False
):
    try:
