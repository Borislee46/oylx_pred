...
def run_qmc_efficiency_test():
    ...
    with tab2:
        ...
        with col_func:
            func_choice = st.selectbox("测试函数", ["Sphere", "Rosenbrock", "Ackley", "Genz's Oscillatory", "Griewank", "Rastrigin"])
        with col_dim:
        ...
        def get_test_func(name):
            if name == "Sphere":
                ...
                return lambda x: np.sum(100.0*(x[:, 1:] - x[:, :-1]**2)**2 + (1 - x[:, :-1])**2, axis=1)
            elif name == "Ackley":
                return lambda x: -20.0 * np.exp(-0.2 * np.sqrt(np.mean(x**2, axis=1))) - \
                ...
                return lambda x: 1 + np.sum(x**2, axis=1)/4000 - np.prod(np.cos(x/np.sqrt(np.arange(1, x.shape[1]+1))), axis=1)
            elif name == "Rastrigin":
    ...
    with tab3:
        ...
        perf_data = []
        for name, eng_cls in [("Sobol", qmc.Sobol), ("Halton", qmc.Halton), ("LHS", qmc.LatinHypercube)]:
