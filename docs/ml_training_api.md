...
（脚本暂不公开训练流水线，有需要请联系lijiapeng8@xdf.cn）
...
- 对采样后的 `sample_weight` 做对齐：若采样器提供 `sample_indices_` 则按索引对齐；对新合成的少数类样本，以该类样本权重均值填充。
...
- 单调约束（XGBoost）：
  - 分类特征约束 0；其余特征通过白名单控制：`MONOTONE_INCREASING_WHITELIST` 设为 +1，`MONOTONE_DECREASING_WHITELIST` 设为 -1，未列入者为 0。
- 两种模式：
...
- 评估结果：`evaluation_results/{model}_evaluation_{timestamp}.json`，包含：
  - `metrics`、`feature_importance`（可选）、`model_params`、`sampling_method`、`calibration_method`、`auto_tune_method`、`package_versions`。
...
- 线上优先加载 `.ubj`，并从 Booster 属性读取 `feature_names/calibration_params/level_fallback_mapping`；若属性缺失，则回退读取同名 JSON（若存在）。文本 TF‑IDF 模型统一采用 `.joblib` 存储（`tfidf_vectorizer.joblib`）。
...
- “正负样本极度不均衡”：优先使用 SMOTENC（含分类索引）；异常时回退到原始数据以保证可用性。
- “需要同时使用采样与样本权重”：已支持。使用 SMOTE/SMOTENC 时，会对新增少数类样本以该类样本权重均值填充。
- “训练环境”：参见 `requirements.txt` 版本；训练脚本会在评估结果中记录 `package_versions`（含 python、xgboost、numpy、pandas、scikit-learn 等）。
