...
V3 的核心洞察——"threshold 应该由业务方拍板而不是从 F1 推导"——是四份报告里最成熟的一个，面试叙事可以直接用。唯一的补充：cost ratio 的敏感性分析假设 FP 和 FN 的代价是固定的，但实际业务中代价可能随用户段位变化（大四冲刺期漏掉机会的代价远大于大一探索期）。如果能做 segment-specific threshold（不同 GPA 段/院校层级的用户用不同阈值），深度更深一层。
