...
DATA_PATH = os.path.join(PROJECT_ROOT, "src", "ml", "data", "cases.feather")
...
def load_and_engineer():
    ...
    df["_no_bg_uni"] = df["background_university"].isna() | (
        df["background_university"].astype(str).str.strip() == ""
    )
    df["_no_bg_major"] = df["background_major"].isna() | (
        df["background_major"].astype(str).str.strip() == ""
    )
    df["_no_target_uni"] = df["target_university"].isna() | (
        df["target_university"].astype(str).str.strip() == ""
    )
    df["_no_target_major"] = df["target_major"].isna() | (
        df["target_major"].astype(str).str.strip() == ""
    )

    gpa_raw = pd.to_numeric(df["gpa"], errors="coerce")
    df["_no_gpa"] = gpa_raw.isna() | (gpa_raw <= 0)

    ielts_ok = pd.to_numeric(df["ielts"], errors="coerce").notna() & (
        pd.to_numeric(df["ielts"], errors="coerce") > 0
    )
    toefl_ok = pd.to_numeric(df["toefl"], errors="coerce").notna() & (
        pd.to_numeric(df["toefl"], errors="coerce") > 0
    )
    df["_no_lang"] = ~(ielts_ok | toefl_ok)

    # Failure classification
    df["_fail_severe"] = (
        df["_no_bg_uni"] | df["_no_bg_major"] | df["_no_target_uni"] | df["_no_target_major"]
    )
    ...
    print(
        f"  Would fail (severe+moderate): {df['_would_fail'].sum()} "
        f"({df['_would_fail'].mean():.1%})"
    )
    print(
        f"    Severe (missing identifiers): {df['_fail_severe'].sum()} "
        f"({df['_fail_severe'].mean():.1%})"
    )
    print(
        f"    Moderate (no GPA & no lang): {df['_fail_moderate'].sum()} "
        f"({df['_fail_moderate'].mean():.1%})"
    )
    print(f"    Mild (no GPA | no lang): {df['_fail_mild'].sum()} ({df['_fail_mild'].mean():.1%})")
...
def analyze_failure_taxonomy(df):
    ...
    for reason, count in sorted(reasons.items(), key=lambda x: x[1], reverse=True):
        ...
        if mask is not None and mask.sum() > 0:
            ...
            print(
                f"  {reason:<25} {count:<8} {count / len(df) * 100:<7.1f}% "
                f"{admit_r:<12.4f} {admit_r - baseline_admit:+.4f}"
            )

    print(
        f"  {'all_ok':<25} {reasons['all_ok']:<8} "
...
def analyze_missing_as_signal(df):
    ...
    results["gpa"] = {
        ...
        "signal_direction": (
            "POSITIVE" if no_gpa["admitted"].mean() > has_gpa["admitted"].mean() else "NEGATIVE"
        ),
    }
    print(
        f"  GPA missing: admit={results['gpa']['no_gpa_admit']:.4f} "
    ...
    results["language"] = {
        ...
        "signal_direction": (
            "POSITIVE" if no_lang["admitted"].mean() > has_lang["admitted"].mean() else "NEGATIVE"
        ),
    }
    print(
        f"  Language missing: admit={results['language']['no_lang_admit']:.4f} "
    ...
    results["bg_major"] = {
        ...
        "signal_direction": (
            "POSITIVE" if no_bgm["admitted"].mean() > has_bgm["admitted"].mean() else "NEGATIVE"
        ),
    }
    print(
        f"  BG Major missing: admit={results['bg_major']['no_bgm_admit']:.4f} "
    ...
    for tier in ["C9", "other", "overseas", "unknown"]:
        ...
        if tier_mask.sum() > 0:
            ...
            print(
                f"    {tier:<10}: missing={miss_rate:.1%}, admit={admit:.4f}, n={tier_mask.sum()}"
            )
    ...
    print("    Has-GPA-at-median students admit: need to check...")
    ...
    if len(near_median) > 0:
        ...
        print(
            f"    Students with GPA≈{gpa_median:.1f} admit: {near_median_admit:.4f} (n={len(near_median)})"
        )
        ...
        print(
            f"    → Median imputation UNDERESTIMATES no-GPA students by "
    ...
    tier_stratified = {}
    print(
        f"  {'Tier':<16} {'N_has_GPA':<10} {'Admit':<8} {'N_no_GPA':<10} {'Admit':<8} {'Δ':<8} {'Pct_no_GPA':<10}"
    )
    print(f"  {'-' * 70}")
    for tier in ["C9", "985", "211", "other_domestic", "overseas", "unknown"]:
        ...
        print(
            f"  {tier:<16} {n_has:<10} {admit_has:.4f}  {n_no:<10} {admit_no:.4f}  "
            f"{admit_no - admit_has:+.4f}  {pct_no:.1f}%"
        )

    # Interpretation
    all_tiers_delta_positive = all(
        tier_stratified[t]["delta"] > 0
        for t in tier_stratified
        if tier_stratified[t]["n_no_gpa"] > 20
    )
    if all_tiers_delta_positive:
        print("\n  ＞ Within EVERY tier, no-GPA students have HIGHER admit rates.")
        print("    This supports the 'missing GPA is a genuine positive signal' hypothesis —")
        print("    it's NOT just a composition effect (Simpson's paradox ruled out).")
    else:
        ...
        print("    The aggregate 13pp gap is partially confounded by school tier.")
...
def analyze_cold_start(df):
    ...
    combo_counts = df[keys].astype(str).groupby(keys, dropna=False).size().reset_index(name="count")
    ...
    for lo, hi, label in [
        ...
        if len(bucket) > 0:
            print(
                f"    n={label:<8}: combos={len(bucket):<6} "
...
def plot_failure_patterns(df, missing_signal, output_path):
    ...
    admit_keys_map = {
        "GPA": ("has_gpa_admit", "no_gpa_admit"),
    ...
    for i, (count, admit) in enumerate(zip(pattern_counts, pattern_admits, strict=False)):
    ...
    level_admits = [
        ...
        float(
            df[df["_fail_mild"] & ~df["_fail_moderate"] & ~df["_fail_severe"]]["admitted"].mean()
        ),
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [f"{lab}\n(n={c})" for lab, c in zip(levels, level_counts, strict=False)], fontsize=9
    )
    ax.set_ylabel("Admission Rate", fontsize=11)
    ax.set_title(
        "Admit Rate by Data Completeness\n(more missing = HIGHER admit?)",
        fontsize=11,
        fontweight="bold",
    )
    for i, (rate, _count) in enumerate(zip(level_admits, level_counts, strict=False)):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [f"{t}\n(n={n})" for t, n in zip(tier_fail["_tier"], tier_fail["n"], strict=False)],
        fontsize=9,
    )
...
def main():
    ...
    plot_failure_patterns(df, missing_signal, os.path.join(OUTPUT_DIR, "failure_patterns.png"))
